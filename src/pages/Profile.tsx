...
const Profile = () => {
    ...
    const deletePost = async (id: string) => {
        ...
        try {
            ...
            toast.success('Recipe deleted forever!');
        } catch (err) {
                    ...
                    <div className="bg-white rounded-3xl p-6 shadow-sm border border-forest-100 sticky top-28">
                        ...
                        <nav className="space-y-2">
                            ...
                            <button onClick={() => setActiveTab('cookbook')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'cookbook' ? 'bg-forest-900 text-white' : 'text-forest-600 hover:bg-forest-50'}`}>
                                <BookOpen className="w-5 h-5" /> My Cookbook
                            </button>
                            <button onClick={() => setActiveTab('orders')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'orders' ? 'bg-forest-900 text-white' : 'text-forest-600 hover:bg-forest-50'}`}>
                                <ShoppingBag className="w-5 h-5" /> Shop Orders
                            </button>
                    ...
                    <AnimatePresence mode="wait">
                        {activeTab === 'dashboard' && (
                            <motion.div key="dash" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                                <h1 className="text-3xl font-black font-display text-forest-900 mb-6">Welcome Back, {user.profile?.name?.split(' ')[0]}!</h1>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                                    <div className="bg-gradient-to-br from-amber-400 to-amber-500 rounded-3xl p-6 text-black shadow-lg">
                                        ...
                                        <p className="text-sm font-bold uppercase tracking-wider mb-1 opacity-80">Lifetime Savings</p>
                                    ...
                                    <div className="bg-white rounded-3xl p-6 shadow-sm border border-forest-100 flex flex-col justify-between">
                                        <div>
                                            <p className="text-sm font-bold text-forest-500 uppercase tracking-wider mb-1">Meal Plan Diet</p>
                                            <p className="text-xl font-black text-forest-900">{user.profile?.dietaryPreference || 'Not Set'}</p>
                                        </div>
                                        <button onClick={() => setActiveTab('settings')} className="text-sm text-forest-500 hover:text-amber-600 font-bold text-left mt-4 underline">Change Diet</button>
                                    </div>
                                    <div className="bg-moss-900 text-white rounded-3xl p-6 shadow-sm flex flex-col justify-center items-start">
                                        <p className="text-sm font-bold text-moss-300 uppercase tracking-wider mb-2">Build Your Plan</p>
                                        <button onClick={() => navigate('/meal-planner')} className="bg-amber-400 text-black px-4 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-amber-300 transition w-full justify-center">
                        ...
                        {activeTab === 'cookbook' && (
                            <motion.div key="cook" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                                    <h2 className="text-3xl font-black font-display text-forest-900">My Cookbook</h2>
                                    <button onClick={() => navigate('/community')} className="bg-forest-100 text-forest-800 px-4 py-2 rounded-xl font-bold hover:bg-forest-200">
                                        Browse Community
                                    </button>
                                </div>
                                ...
                                <div className="bg-white rounded-3xl p-6 shadow-sm border border-forest-100 min-h-[400px]">
                                    {loadingPosts ? (
                                            ...
                                            {myPosts.map(post => (
                                                <div key={post.id} className="flex justify-between items-center p-4 border border-forest-50 hover:bg-cream-50 rounded-2xl transition">
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-forest-500">{new Date(post.created_at).toLocaleDateString()} • {post.likes} Likes • {post.recipe_ingredients.length} Ingredients</p>
                                                    </div>
                                        ...
                                        <div className="text-center py-20">
                                            ...
                                            <p className="text-forest-600 font-medium mb-4">You haven't shared any recipes yet!</p>
                                            <button onClick={() => navigate('/community')} className="bg-forest-900 text-cream-50 px-6 py-3 rounded-xl font-bold hover:bg-forest-800">Post a Recipe</button>
                                        </div>
                                    )}
                                </div>
                            </motion.div>
                        ...
                        {activeTab === 'orders' && (
                            <motion.div key="orders" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                                <h2 className="text-3xl font-black font-display text-forest-900 mb-6">Mushroom Shop Orders</h2>
                                <div className="bg-white rounded-3xl shadow-sm border border-forest-100 p-8">
                                    {user?.orders && user.orders.length > 0 ? (
                                            ...
                                            {user.orders.map((order) => (
                                                <div key={order.id} className="border border-forest-50 bg-cream-50 rounded-2xl p-6">
                                                    ...
                                                    <div className="flex justify-between items-center pt-4 border-t border-forest-100">
                                                        <span className="text-forest-600 font-medium">{order.items.length} items</span>
                                        ...
                                        <div className="text-center py-16">
                                            ...
                                            <p className="text-forest-500 mb-4">No premium mushroom orders yet.</p>
                                            <button onClick={() => navigate('/mushroom-shop')} className="text-amber-600 font-bold hover:underline">Visit the Shop</button>
                                        </div>
                                    )}
                                </div>
                            </motion.div>
                        ...
                        {activeTab === 'settings' && (
                            <motion.div key="settings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                                <h2 className="text-3xl font-black font-display text-forest-900 mb-6">User Settings</h2>
                                <form onSubmit={handleSaveSettings} className="bg-white rounded-3xl p-8 shadow-sm border border-forest-100 max-w-2xl">
                                    <div className="flex items-center gap-3 p-4 bg-blue-50 text-blue-800 rounded-2xl mb-8">
                                        ...
                                        <p className="text-sm font-medium">These settings are automatically used by Chef Aika and the Meal Planner.</p>
                                    </div>
                                    
                                    <div className="space-y-6">
                                        <div>
                                            <label className="block text-sm font-bold text-forest-700 mb-2">Dietary Preference</label>
                                            <select
                                        ...
                                        <div>
                                            <label className="block text-sm font-bold text-forest-700 mb-2">Default Family Size</label>
