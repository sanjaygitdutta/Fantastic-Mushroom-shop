      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 bg-moss-100 text-moss-800 text-sm font-bold px-4 py-1.5 rounded-full mb-4">
            <Leaf className="w-4 h-4" /> {t('seas_badge', { defaultValue: 'Field to Fork' })}
          </div>
        ...
        <div className="bg-linear-to-r from-moss-700 to-forest-800 rounded-3xl p-6 mb-8 text-white">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              ...
              <h2 className="text-2xl font-black font-display">{currentItems.length} {t('seas_freshest', { defaultValue: 'items at their freshest & cheapest!' })}</h2>
            </div>
        ...
        <div className="flex overflow-x-auto gap-2 pb-3 mb-6 scrollbar-hide">
          {MONTH_NAMES.map((month, i) => {
            ...
            return (
              ...
              >
                {month.slice(0, 3)}
                <span className="block text-xs font-normal opacity-70">{count} {t('seas_items', { defaultValue: 'items' })}</span>
              </button>
            );
          })}
        </div>
        ...
        <div className="flex gap-2 mb-6">
          {(['all', 'vegetable', 'fruit'] as const).map(f => (
            ...
            >
              ...
              {f === 'all' ? t('seas_filter_all', { defaultValue: 'All' }) : f === 'vegetable' ? t('seas_filter_vegetables', { defaultValue: 'Vegetables' }) : t('seas_filter_fruits', { defaultValue: 'Fruits' })}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {items.length === 0 ? (
            <div className="col-span-full text-center py-12 text-forest-500">
              {t('seas_no_items', { defaultValue: 'No items found for this selection.' })}
            </div>
          ) : items.map((item, i) => (
            <motion.div
              key={item.name}
            ...
            >
              ...
              <span className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded-full mb-3 ${
                ...
                {item.type === 'vegetable' ? t('seas_type_vegetable', { defaultValue: '🥦 Vegetable' }) : t('seas_type_fruit', { defaultValue: '🍎 Fruit' })}
              </span>
              ...
              >
                {t('seas_compare_price', { defaultValue: 'Compare Price' })} <ArrowRight className="w-3 h-3" />
              </Link>
            </motion.div>
          ))}
        </div>
