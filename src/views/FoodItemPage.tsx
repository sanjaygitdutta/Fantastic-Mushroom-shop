  ...
  return (
    <div className="min-h-screen bg-cream-50 pt-24 pb-16">
      ...
      <div className="max-w-6xl mx-auto px-4 mb-14">
        <h2 className="text-xl font-bold text-forest-900 mb-6">{t('food_item_live_compare')} — {translatedItem}</h2>
        {loading ? (
          ...
          <>
            ...
            <div className="bg-forest-50 p-6 rounded-2xl border border-forest-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 max-w-3xl mx-auto mt-8">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-forest-900 font-bold mb-1">{t('food_item_set_alert')}</h3>
                  <p className="text-forest-600 text-sm">{t('food_item_alert_desc')}</p>
                </div>
              </div>

              {alertSuccess ? (
                <div className="w-full md:w-auto bg-green-100 text-green-800 px-4 py-3 rounded-xl font-medium border border-green-200">
                  ✅ {t('food_item_alert_success')}
                </div>
              ) : (
                <form onSubmit={handleAlertSubmit} className="w-full md:w-auto flex flex-col gap-2">
                  <div className="flex gap-2">
                    <input
                      type="email"
                      placeholder={t('food_item_enter_email')}
                      required
                      value={alertEmail}
                    ...
                    >
                      {alertSubmitting ? t('food_item_saving') : t('food_item_notify_me')}
                    </button>
                  </div>
          ...
          <div className="bg-white p-12 text-center rounded-3xl border border-forest-100">
            <h3 className="text-xl font-bold text-forest-900 mb-2">{t('food_item_not_found')}</h3>
            <p className="text-forest-600">{t('food_item_not_found_desc', { foodItem })}</p>
          </div>
        )}
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 mb-14">
        <h2 className="text-2xl font-black font-display text-forest-900 mb-6">
          {t('food_item_faq')} — {translatedItem}
        </h2>
        <div className="space-y-4">
          {[
            {
              q: t('food_item_faq_q1', { displayName }),
              a: t('food_item_faq_a1', { displayName }),
            },
            {
              q: t('food_item_faq_q2', { displayName }),
              a: t('food_item_faq_a2', { displayName }),
            },
            {
              q: t('food_item_faq_q3', { displayName }),
              a: t('food_item_faq_a3', { displayName }),
            },
            {
              q: t('food_item_faq_q4', { displayName }),
              a: t('food_item_faq_a4', { displayName }),
            },
          ].map(({ q, a }) => (
      ...
      <div className="max-w-6xl mx-auto px-4">
        <h2 className="text-xl font-black font-display text-forest-900 mb-4">{t('food_item_related')}</h2>
        <div className="flex flex-wrap gap-2">
