      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-black font-display text-forest-900 flex items-center gap-2">
              <Bell className="w-6 h-6 text-amber-500" /> {t('saved_title', { defaultValue: 'Watchlist' })}
            </h1>
            <p className="text-forest-600 text-sm mt-1">{t('saved_subtitle', { defaultValue: 'Items you are tracking for price drops.' })}</p>
          </div>
          <Link href="/compare" className="btn-forest flex items-center gap-2 text-sm px-4 py-2">
            <Search className="w-4 h-4" /> {t('saved_add_items', { defaultValue: 'Add Items' })}
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white p-12 rounded-3xl border border-forest-100 text-center shadow-sm">
            ...
            <h2 className="text-xl font-bold text-forest-900 mb-2">{t('saved_empty_title', { defaultValue: 'Your watchlist is empty' })}</h2>
            <p className="text-forest-600 mb-6 max-w-sm mx-auto">{t('saved_empty_desc', { defaultValue: 'Track your favorite groceries to get notified when prices drop across Quick Commerce platforms.' })}</p>
            <Link href="/compare?q=milk" className="btn-amber inline-flex items-center gap-2">
              {t('saved_start_tracking', { defaultValue: 'Start Tracking Prices' })}
            </Link>
          </div>
        ) : (
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 text-xs text-forest-500 mt-1">
                      <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {t('saved_tracked_since', { defaultValue: 'Tracked since' })} {new Date(alert.created_at).toLocaleDateString()}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between sm:justify-end gap-6 sm:w-auto w-full border-t sm:border-t-0 border-forest-50 pt-4 sm:pt-0">
                  ...
                  <div className="flex items-center gap-2">
                    <Link href={`/compare?q=${encodeURIComponent(alert.item_query)}`} className="bg-forest-100 hover:bg-forest-200 text-forest-800 px-4 py-2 rounded-xl text-sm font-bold transition-colors">
                      {t('saved_check_live', { defaultValue: 'Check Live Price' })}
                    </Link>
                    <button 
                      onClick={() => deleteAlert(alert.id)}
                      className="p-2 text-red-400 hover:bg-red-50 hover:text-red-500 rounded-xl transition-colors"
                      title={t('saved_remove_title', { defaultValue: 'Remove from Watchlist' })}
                    >
